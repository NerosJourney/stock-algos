...
def init_stocks_table():
    cur = db.cursor()
    create_stocks_table = 'CREATE TABLE Stocks (ticker VARCHAR(6) PRIMARY KEY NOT NULL, curr_price DECIMAL(6,2), open DECIMAL(6,2), last_close DECIMAL(6,2))'
...
def create_stock_rows(file_name):
    ...
    for stock in stocks:
        try:
            cur.execute(f'INSERT INTO Stocks (ticker, curr_price, open, last_close) VALUES ("{stock}", 0, 0, 0)')
        except:
